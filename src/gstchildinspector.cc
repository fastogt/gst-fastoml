...
typedef gchar* (*GstChildInspectorTypeToString)(GParamSpec* pspec, GValue* value);

static gchar* gst_child_inspector_type_int_to_string(GParamSpec* pspec, GValue* value);
static gchar* gst_child_inspector_type_string_to_string(GParamSpec* pspec, GValue* value);
...
static GstChildInspectorFlag flags[] = {{G_PARAM_READABLE, "readable"}, {G_PARAM_WRITABLE, "writable"}, {}};

static GstChildInspectorType types[] = {{G_TYPE_INT, gst_child_inspector_type_int_to_string},
...
static gchar* gst_child_inspector_type_int_to_string(GParamSpec* pspec, GValue* value) {
  ...
  return g_strdup_printf("Integer. Range: %d - %d Default: %d", pint->minimum, pint->maximum, g_value_get_int(value));
}

static const gchar* gst_child_inspector_flag_to_string(gint flag) {
  for (GstChildInspectorFlag* current_flag = flags; current_flag->to_string; ++current_flag) {
    if (current_flag->value == flag) {
      return current_flag->to_string;
    }
  }
...
static gchar* gst_child_inspector_type_to_string(GParamSpec* pspec, GValue* value) {
...
gchar* gst_child_inspector_property_to_string(GObject* object, GParamSpec* param, guint alignment) {
  ...
  prop = g_strdup_printf(
      ...
      "%*s%-21.21s flags: %s\n"
      "%*s%-21.21s %s",
      alignment, "", name, blurb, alignment, "", "", flags, alignment, "", "", type);
...
gchar* gst_child_inspector_properties_to_string(GObject* object, guint alignment, gchar* title) {
  ...
  property_specs = g_object_class_list_properties(G_OBJECT_GET_CLASS(object), &num_properties);

  if (title)
    props = title;

  for (i = 0; i < num_properties; i++) {
    prop = gst_child_inspector_property_to_string(object, property_specs[i], alignment);
    if (props) {
      gchar* tmp;
