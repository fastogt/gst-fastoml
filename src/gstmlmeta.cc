...
static gboolean gst_classification_meta_init(GstMeta* meta, gpointer params, GstBuffer* buffer);
...
static gboolean gst_detection_meta_init(GstMeta* meta, gpointer params, GstBuffer* buffer);
...
static gboolean gst_detection_meta_copy(GstBuffer* transbuf, GstMeta* meta, GstBuffer* buffer);
static gboolean gst_detection_meta_scale(GstBuffer* transbuf,
                                         GstMeta* meta,
                                         GstBuffer* buffer,
                                         GstVideoMetaTransform* data);
static gboolean gst_classification_meta_copy(GstBuffer* transbuf, GstMeta* meta, GstBuffer* buffer);
...
const GstMetaInfo* gst_embedding_meta_get_info(void) {
  ...
  if (g_once_init_enter(&embedding_meta_info)) {
    const GstMetaInfo* meta = gst_meta_register(GST_EMBEDDING_META_API_TYPE, "GstEmbeddingMeta",
                                                sizeof(GstEmbeddingMeta), gst_classification_meta_init,
                                                gst_classification_meta_free, gst_classification_meta_transform);
...
const GstMetaInfo* gst_classification_meta_get_info(void) {
  ...
  if (g_once_init_enter(&classification_meta_info)) {
    const GstMetaInfo* meta = gst_meta_register(GST_CLASSIFICATION_META_API_TYPE, "GstClassificationMeta",
                                                sizeof(GstClassificationMeta), gst_classification_meta_init,
                                                gst_classification_meta_free, gst_classification_meta_transform);
...
GType gst_detection_meta_api_get_type(void) {
  static volatile GType type = 0;
  static const gchar* tags[] = {GST_META_TAG_VIDEO_STR, GST_META_TAG_VIDEO_ORIENTATION_STR, GST_META_TAG_VIDEO_SIZE_STR,
                                NULL};
...
const GstMetaInfo* gst_detection_meta_get_info(void) {
  ...
  if (g_once_init_enter(&detection_meta_info)) {
    const GstMetaInfo* meta =
        gst_meta_register(GST_DETECTION_META_API_TYPE, "GstDetectionMeta", sizeof(GstDetectionMeta),
                          gst_detection_meta_init, gst_detection_meta_free, gst_detection_meta_transform);
...
static gboolean gst_detection_meta_copy(GstBuffer* dest, GstMeta* meta, GstBuffer* buffer) {
  ...
  dmeta = (GstDetectionMeta*)gst_buffer_add_meta(dest, GST_DETECTION_META_INFO, NULL);
  if (!dmeta) {
...
static gboolean gst_detection_meta_scale(GstBuffer* dest,
                                         GstMeta* meta,
                                         GstBuffer* buffer,
  ...
  for (gint i = 0; i < dmeta->num_boxes; ++i) {
    ...
    GST_LOG("scaled bbox %d: %fx%f@%fx%f -> %fx%f@%fx%f", smeta->boxes[i].label, smeta->boxes[i].x, smeta->boxes[i].y,
            smeta->boxes[i].width, smeta->boxes[i].height, dmeta->boxes[i].x, dmeta->boxes[i].y, dmeta->boxes[i].width,
            dmeta->boxes[i].height);
  }
  return TRUE;
}

static gboolean gst_detection_meta_transform(GstBuffer* dest,
                                             GstMeta* meta,
                                             GstBuffer* buffer,
                                             GQuark type,
                                             gpointer data) {
...
static gboolean gst_classification_meta_copy(GstBuffer* dest, GstMeta* meta, GstBuffer* buffer) {
  GstClassificationMeta *dmeta, *smeta;
  gsize raw_size;

  smeta = (GstClassificationMeta*)meta;
  dmeta = (GstClassificationMeta*)gst_buffer_add_meta(dest, GST_CLASSIFICATION_META_INFO, NULL);
