...
static GstStaticPadTemplate sink_bypass_factory =
    GST_STATIC_PAD_TEMPLATE("sink_bypass", GST_PAD_SINK, GST_PAD_REQUEST, GST_STATIC_CAPS("ANY"));

static GstStaticPadTemplate src_bypass_factory =
    GST_STATIC_PAD_TEMPLATE("src_bypass", GST_PAD_SRC, GST_PAD_REQUEST, GST_STATIC_CAPS("ANY"));
...
enum { PROP_0, PROP_BACKEND, PROP_MODEL_LOCATION };
...
static void gst_video_inference_set_property(GObject* object,
                                             ...
                                             const GValue* value,
                                             GParamSpec* pspec);
static void gst_video_inference_get_property(GObject* object, guint property_id, GValue* value, GParamSpec* pspec);

/* GstElement methods */
static GstStateChangeReturn gst_video_inference_change_state(GstElement* element, GstStateChange transition);
...
static void gst_video_inference_child_proxy_init(GstChildProxyInterface* iface);
static GObject* gst_video_inference_get_child_by_name(GstChildProxy* parent, const gchar* name);
static GObject* gst_video_inference_get_child_by_index(GstChildProxy* parent, guint index);
...
static GstFlowReturn gst_video_inference_collected(GstCollectPads* pads, gpointer user_data);
static GstFlowReturn gst_video_inference_pop_buffer(GstVideoInference* self,
                                                    GstCollectPads* cpads,
                                                    GstCollectData* data,
                                                    GstBuffer** buffer);
static GstFlowReturn gst_video_inference_forward_buffer(GstVideoInference* self, GstBuffer* buffer, GstPad* pad);
static gboolean gst_video_inference_model_buffer_process(GstVideoInference* self,
...
static gboolean gst_video_inference_postprocess(GstVideoInference* self,
                                                GstVideoInferenceClass* klass,
                                                const gpointer prediction_data,
                                                gsize prediction_size,
                                                GstBuffer* buffer_model,
                                                GstVideoInferencePad* pad_model,
                                                GstBuffer* buffer_bypass,
                                                GstVideoInferencePad* pad_bypass);

static GstIterator* gst_video_inference_iterate_internal_links(GstPad* pad, GstObject* parent);
static gboolean gst_video_inference_sink_event(GstCollectPads* pads,
                                               GstCollectData* pad,
                                               GstEvent* event,
                                               gpointer user_data);
static gboolean gst_video_inference_src_event(GstPad* pad, GstObject* parent, GstEvent* event);
static GstPad* gst_video_inference_get_src_pad(GstVideoInference* self, GstVideoInferencePrivate* priv, GstPad* pad);
static GstPad* gst_video_inference_get_sink_pad(GstVideoInference* self, GstVideoInferencePrivate* priv, GstPad* pad);
static void gst_video_inference_set_backend(GstVideoInference* self, gint backend);
...
static void video_inference_map_buffers(GstVideoInferencePad* data,
                                        ...
                                        GstVideoFrame* inframe,
                                        GstVideoFrame* outframe);
static gboolean video_inference_prepare_postprocess(const GstMetaInfo* meta_info,
                                                    GstBuffer* buffer,
                                                    GstVideoInfo* video_info,
                                                    GstVideoFrame* out_frame,
                                                    GstMeta** out_meta);
static void video_inference_buffer_unref(GstBuffer* buffer);
static void video_inference_frame_unmap(GstBuffer* buffer, GstVideoFrame* frame);
...
G_DEFINE_TYPE_WITH_CODE(GstVideoInference,
                        gst_video_inference,
                        GST_TYPE_ELEMENT,
                        ...
                        G_IMPLEMENT_INTERFACE(GST_TYPE_CHILD_PROXY, gst_video_inference_child_proxy_init));

#define GST_VIDEO_INFERENCE_PRIVATE(self) (GstVideoInferencePrivate*)(gst_video_inference_get_instance_private(self))

static void gst_video_inference_class_init(GstVideoInferenceClass* klass) {
  ...
  backend_blurb = g_strdup_printf(
      "Type of predefined backend to use.\n"
  ...
  g_object_class_install_property(oclass, PROP_BACKEND,
                                  g_param_spec_enum("backend", "Backend", backend_blurb, GST_TYPE_INFERENCE_BACKENDS,
                                                    gst_inference_backends_get_default_backend(), G_PARAM_READWRITE));

  g_object_class_install_property(oclass, PROP_MODEL_LOCATION,
                                  g_param_spec_string("model-location", "Model Location", "Path to the model to use",
                                                      DEFAULT_MODEL_LOCATION, G_PARAM_READWRITE));

  gst_video_inference_signals[NEW_PREDICTION_SIGNAL] =
      g_signal_new("new-prediction", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST, 0, NULL, NULL, NULL, G_TYPE_NONE, 4,
...
static void gst_video_inference_init(GstVideoInference* self) {
  ...
  gst_collect_pads_set_function(priv->cpads, gst_video_inference_collected, (gpointer)(self));
  gst_collect_pads_set_event_function(priv->cpads, gst_video_inference_sink_event, (gpointer)(self));

  priv->model_location = g_strdup(DEFAULT_MODEL_LOCATION);

  gst_video_inference_set_backend(self, gst_inference_backends_get_default_backend());
}
...
static gboolean gst_video_inference_start(GstVideoInference* self) {
  ...
  if (NULL == priv->model_location) {
    GST_ELEMENT_ERROR(self, RESOURCE, NOT_FOUND, ("Model Location has not been set"), (NULL));
    ret = FALSE;
    goto out;
  }

  if (!gst_backend_start(priv->backend, priv->model_location, &err)) {
    GST_ELEMENT_ERROR(self, LIBRARY, INIT, ("Could not start the selected backend: (%s)", err->message), (NULL));
    ret = FALSE;
  }
...
static gboolean gst_video_inference_stop(GstVideoInference* self) {
  ...
  if (!gst_backend_stop(priv->backend, &err)) {
    GST_ELEMENT_ERROR(self, LIBRARY, INIT, ("Could not stop the selected backend: (%s)", err->message), (NULL));
    ret = FALSE;
  }
...
static GstStateChangeReturn gst_video_inference_change_state(GstElement* element, GstStateChange transition) {
  ...
  ret = GST_ELEMENT_CLASS(gst_video_inference_parent_class)->change_state(element, transition);
...
static GstPad* gst_video_inference_create_pad(GstVideoInference* self,
  ...
  if (GST_PAD_IS_SINK(pad)) {
    ...
    *data = (GstVideoInferencePad*)gst_collect_pads_add_pad(priv->cpads, pad, sizeof(GstVideoInferencePad), NULL, TRUE);
    if (NULL == *data) {
  ...
  gst_pad_set_iterate_internal_links_function(pad, gst_video_inference_iterate_internal_links);
...
static GstFlowReturn gst_video_inference_forward_buffer(GstVideoInference* self, GstBuffer* buffer, GstPad* pad) {
  ...
  GST_LOG_OBJECT(self, "Forwarding buffer %" GST_PTR_FORMAT " to %" GST_PTR_FORMAT, buffer, pad);
  ...
  GST_CAT_LEVEL_LOG(GST_CAT_DEFAULT, level, self, "Pad %" GST_PTR_FORMAT " returned: (%d) %s", pad, ret,
                    gst_flow_get_name(ret));

  return ret;
}

static void video_inference_map_buffers(GstVideoInferencePad* cpad,
                                        GstBuffer* inbuf,
                                        GstVideoFrame* inframe,
                                        GstVideoFrame* outframe) {
  GstVideoInfo* info;
...
static gboolean gst_video_inference_preprocess(GstVideoInference* self,
  ...
  if (NULL == klass->preprocess) {
    GST_ELEMENT_ERROR(self, STREAM, FAILED, ("Subclass did not implement preprocess"), (NULL));
    return FALSE;
  }
  ...
  if (!klass->preprocess(self, inframe, outframe)) {
    GST_ELEMENT_ERROR(self, STREAM, FAILED, ("Subclass failed to preprocess"), (NULL));
    return FALSE;
  }
...
static gboolean gst_video_inference_predict(GstVideoInference* self,
  ...
  if (!gst_backend_process_frame(priv->backend, frame, pred, pred_size, &error)) {
    GST_ELEMENT_ERROR(self, STREAM, FAILED, ("Could not process using the selected backend: (%s)", error->message),
                      (NULL));
    g_error_free(error);
    return FALSE;
  }
...
static gboolean gst_video_inference_model_buffer_process(GstVideoInference* self,
  ...
  video_inference_map_buffers(priv->sink_model_data, buffer, &inframe, &outframe);
  ...
  if (!gst_video_inference_predict(self, priv, &outframe, prediction_data, prediction_size)) {
    ret = FALSE;
    goto free_frames;
  }
...
static GstMeta* video_inference_transform_meta(GstBuffer* buffer_model,
  ...
  GQuark orientation_quark = g_quark_from_static_string(GST_META_TAG_VIDEO_ORIENTATION_STR);
  ...
  if (gst_meta_api_type_has_tag(info->api, size_quark) || gst_meta_api_type_has_tag(info->api, orientation_quark)) {
    GstVideoMetaTransform trans = {info_model, info_bypass};

    info->transform_func(buffer_bypass, meta_model, buffer_model, scale_quark, &trans);
  } else {
    info->transform_func(buffer_bypass, meta_model, buffer_model, copy_quark, NULL);
  }
...
static gboolean gst_video_inference_postprocess(GstVideoInference* self,
  ...
  if (!video_inference_prepare_postprocess(klass->inference_meta_info, buffer_model, info_model, &frame_model,
                                           &meta_model)) {
    return FALSE;
  }

  if (!video_inference_prepare_postprocess(klass->inference_meta_info, buffer_bypass, info_bypass, &frame_bypass,
                                           NULL)) {
    return FALSE;
  }

  GST_LOG_OBJECT(self, "Calling frame postprocess");
  if (!klass->postprocess(self, prediction_data, prediction_size, meta_model, info_model, &pred_valid)) {
    GST_ELEMENT_ERROR(self, STREAM, FAILED, ("Subclass failed at preprocess"), (NULL));
    return FALSE;
  }
  ...
  if (pred_valid) {
    ...
    meta_bypass = video_inference_transform_meta(buffer_model, info_model, meta_model, buffer_bypass, info_bypass);
    g_signal_emit(self, gst_video_inference_signals[NEW_PREDICTION_SIGNAL], 0, meta_model, &frame_bypass, meta_bypass,
                  pbpass);
  } else {
...
static GstFlowReturn gst_video_inference_pop_buffer(GstVideoInference* self,
  ...
  *buffer = gst_buffer_make_writable(*buffer);
  ...
  GST_LOG_OBJECT(self, "Popped %" GST_PTR_FORMAT " from %" GST_PTR_FORMAT, *buffer, data->pad);
  return GST_FLOW_OK;
}
...
static GstFlowReturn gst_video_inference_collected(GstCollectPads* pads, gpointer user_data) {
  ...
  ret = gst_video_inference_pop_buffer(self, pads, (GstCollectData*)priv->sink_model_data, &buffer_model);
  if (GST_FLOW_OK != ret) {
    goto out;
  }

  ret = gst_video_inference_pop_buffer(self, pads, (GstCollectData*)priv->sink_bypass_data, &buffer_bypass);
  if (GST_FLOW_OK != ret) {
    goto model_free;
  }
  ...
  if (buffer_model) {
    ...
    if (!gst_video_inference_model_buffer_process(self, klass, priv, buffer_model, &prediction_data,
                                                  &prediction_size)) {
      ret = GST_FLOW_ERROR;
      goto bypass_free;
    }
    ...
    if (!gst_video_inference_postprocess(self, klass, prediction_data, prediction_size, buffer_model,
                                         priv->sink_model_data, buffer_bypass, priv->sink_bypass_data)) {
      ret = GST_FLOW_ERROR;
      goto bypass_free;
    }
  }
  ...
  ret = gst_video_inference_forward_buffer(self, buffer_model, priv->src_model);
  ...
  ret = gst_video_inference_forward_buffer(self, buffer_bypass, priv->src_bypass);
...
static void gst_video_inference_set_caps(GstVideoInference* self,
  ...
  if (gst_caps_is_fixed(caps)) {
    ...
    GST_INFO_OBJECT(self, "Updating caps in %" GST_PTR_FORMAT " to %" GST_PTR_FORMAT, data->pad, caps);
...
static gboolean gst_video_inference_sink_event(GstCollectPads* pads,
  ...
  GST_LOG_OBJECT(self, "Received event %s from %" GST_PTR_FORMAT, GST_EVENT_TYPE_NAME(event), pad->pad);
  ...
  if (NULL != srcpad) {
    GST_LOG_OBJECT(self, "Forwarding event %s from %" GST_PTR_FORMAT, GST_EVENT_TYPE_NAME(event), pad->pad);
    /* Collect pads will decrease the refcount of the event when we return */
    gst_event_ref(event);
    if (FALSE == gst_pad_push_event(srcpad, event)) {
      GST_ERROR_OBJECT(self, "Event %s failed in %" GST_PTR_FORMAT, GST_EVENT_TYPE_NAME(event), srcpad);
      goto out;
    }
  } else {
    GST_LOG_OBJECT(self, "Dropping event %s from %" GST_PTR_FORMAT, GST_EVENT_TYPE_NAME(event), pad->pad);
  }
